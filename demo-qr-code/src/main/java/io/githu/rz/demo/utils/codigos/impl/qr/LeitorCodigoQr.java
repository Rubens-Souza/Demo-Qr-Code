...
public class LeitorCodigoQr<T> implements ILeitorCodigo<T> {
	...
	private String removerCabecalhoBase64(String base64) {
		final String regexPadraoCabecalhoBase64 = "^data:image\\/[a-zA-Z]+;base64,";
		return base64.replace(regexPadraoCabecalhoBase64, StringUtils.EMPTY);
	}
	...
	private BinaryBitmap converterBase64(String base64) {
		...
		BufferedImage imagemCodigoQr;
		try {
			imagemCodigoQr = ImageIO.read(inputStreamImagem);
		} catch (IOException ex) {
			throw new LeituraCodigoException(String.format(MSG_ERRO_LEITURA_CODIGO_QR, ex.getMessage()));
		}
		
		LuminanceSource sourceImagem = new BufferedImageLuminanceSource(imagemCodigoQr);
	    BinaryBitmap bitmap = new BinaryBitmap(new HybridBinarizer(sourceImagem));
	    return bitmap;
	}
